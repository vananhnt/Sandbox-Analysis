...
class Analysis:
    ...
    def check_result_output(self):
        """ Check the result of sample generated by LLM"""
                        ...
                        with open(self.storage_path + str(analysis_id) + '/reports/report.json', 'rb') as f:
                            ...
                            for index in range(0, len(call_ids)):
                                if call_apis[index] == 'NtWriteFile':
                                    log = re.sub(r'\\x[0-9a-fA-F]{2}', '', call_argument_list[index][2]['value'])
                                    if log != '':
                                        raw_prompts.append(log)
                        
                            for raw_prompt in raw_prompts:
                                prompt = prompt + raw_prompt
                            
                            malstatus = json_report_data['malstatus']
                            malscore = json_report_data['malscore']
                else:
                    if self.log_level > 0:
